...
(function(gmu, $, undefined) {
  ...
  gmu.define('Refresh', {
      ...
      me.on('ready', function() {
        $.each(['up', 'down'], function(i, dir) {
          ...
          if ($elem.length) {
            ...
            if (!$elem.find('.ui-refresh-icon').length || !$elem.find('.ui-refresh-label').length) {
              me._createBtn(dir);
            }
    ...
    _createBtn: function(dir) {
      ...
      var txt = $.trim($elem.text()) || opts.btnText;

      $elem.html('<i class="ui-refresh-icon"></i><span class="ui-refresh-label">' + txt + '</span>');

      opts.btnClass && $elem.addClass(opts.btnClass);
    ...
    _changeStyle: function(dir, state) {
      ...
      switch (state) {
        case 'loaded':
          this._options['$' + dir + 'Elem'].removeClass('loading');
          refreshInfo['$label'].html(refreshInfo['text']);
          refreshInfo['$icon'].removeClass();
          opts._actDir = '';
          break;
        case 'loading':
          this._options['$' + dir + 'Elem'].addClass('loading');
      ...
      if (!container.length) {
        ...
        if (!opts.content) {
          ...
          if ($el.is('ul') || $el.is('ol')) {
            ...
            // if (container[0].id) {
            //   $el[0].id = container[0].id;
            //   $el[0].className = container[0].className;
            //   container.removeAttr('id').removeAttr('class');
            // }
          } else {
